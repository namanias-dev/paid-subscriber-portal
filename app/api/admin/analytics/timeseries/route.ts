...
import { getAnalyticsTimeseries, resolveRange, type RangePreset } from "@/lib/analytics/queries";
...
export async function GET(req: Request) {
  try {
    ...
    const timeseries = await getAnalyticsTimeseries({ from, to, excludeAdmin });
    return NextResponse.json({ ok: true, timeseries });
  } catch {
