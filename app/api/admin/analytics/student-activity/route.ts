...
import { getStudentActivity, resolveRange, type RangePreset } from "@/lib/analytics/queries";
...
export async function GET(req: Request) {
  try {
    ...
    const activity = await getStudentActivity({ from, to, excludeAdmin });
    return NextResponse.json({ ok: true, activity });
  } catch {
    return NextResponse.json({ ok: false, error: "Failed to load student activity." }, { status: 500 });
  }
}
