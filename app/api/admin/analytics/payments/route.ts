...
import { getPaymentIntelligence, resolveRange, type RangePreset } from "@/lib/analytics/queries";
...
export async function GET(req: Request) {
  try {
    ...
    const intelligence = await getPaymentIntelligence({ from, to, excludeAdmin });
    return NextResponse.json({ ok: true, intelligence });
  } catch {
    return NextResponse.json({ ok: false, error: "Failed to load payment intelligence." }, { status: 500 });
  }
}
