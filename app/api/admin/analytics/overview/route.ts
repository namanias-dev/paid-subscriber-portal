...
import { getAnalyticsOverview, resolveRange, type RangePreset } from "@/lib/analytics/queries";
...
export async function GET(req: Request) {
  try {
    ...
    const overview = await getAnalyticsOverview({ from, to, excludeAdmin });
    return NextResponse.json({ ok: true, overview });
  } catch {
    return NextResponse.json({ ok: false, error: "Failed to load analytics overview." }, { status: 500 });
  }
}
