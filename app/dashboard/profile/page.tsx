...
export default function ProfilePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <h1 className="font-heading text-2xl text-text">Profile</h1>

      <div className="card p-5">
        <div className="flex items-center justify-between">
          <div>
            <p className="font-heading text-xl text-text">{student.name}</p>
        ...
        <button onClick={() => setRenewOpen(true)} className="btn-gold mt-5 w-full">
          Renew / Upgrade Plan
        </button>
      </div>

      <div className="card p-5">
        <h3 className="mb-3 font-heading text-lg text-text">UPSC Preferences</h3>
...
function Info({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-text ${mono ? "font-mono text-gold-light" : ""}`}>{value}</p>
    </div>
  );
}
