...
// ---------------------------------------------------------------------------
function buildInstallmentReminders(): BuilderGraph {
  const nodes: BuilderNode[] = [
    ...
    smsNode("sms_overdue", "Overdue reminder", "payment_reminder", undefined, {}, pos(260, 0), "installment_overdue_reminder"),
    ...
    smsNode("sms_final", "Final reminder", "payment_reminder", undefined, {}, pos(1040, 140), "installment_final_reminder"),
...
function buildWebinarOnboarding(byKey: Map<string, AutomationTemplateOption>): BuilderGraph {
  ...
  const nodes: BuilderNode[] = [
    ...
    { node_key: "wait_1d", type: "wait", position: pos(520, 0), config: { title: "Wait 1 day", durationValue: 1, durationUnit: "days" } },
    smsNode("sms_join", "How to join tutorial", "transactional", undefined, {}, pos(780, 0), "webinar_join_tutorial"),
    ...
    smsNode("sms_dayof", "Day-of reminder", "transactional", undefined, {}, pos(1560, 140), "webinar_day_of_reminder"),
...
export const JOURNEY_DEFS: JourneyDef[] = [
  { name: "Installment Reminders", description: "Overdue installment dunning; auto-stops the instant the student pays.", build: () => buildInstallmentReminders() },
...
export async function seedJourneySet(actor: KillSwitchActor): Promise<SeedJourneyResult[]> {
  ...
  for (const def of JOURNEY_DEFS) {
    ...
    const wf = await createWorkflow(def.name, actor);
    await saveDraftGraph(wf.id, graph, actor, `Seeded "${def.name}" journey (draft; execution off)`);
    results.push({ name: def.name, created: true, workflowId: wf.id, pendingTemplateKeys: pk });
  }
  return results;
}
