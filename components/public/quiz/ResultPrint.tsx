...
export default function ResultPrint({ attemptId }: { attemptId: string }) {
  ...
  return (
    <div style={{ maxWidth: 820, margin: "0 auto", padding: 28, fontFamily: "system-ui, sans-serif", color: "#0f172a" }}>
      ...
      <div style={{ borderBottom: "3px solid #1e3a8a", paddingBottom: 12, marginBottom: 16 }}>
        <h1 style={{ margin: 0, fontSize: 22, color: "#1e3a8a" }}>Naman IAS Academy</h1>
      ...
      {data.questions.map((qq, i) => (
        <div key={i} style={{ borderBottom: "1px solid #e2e8f0", padding: "10px 0", fontSize: 13, breakInside: "avoid" }}>
          <div style={{ display: "flex", gap: 6 }}>
            <b>Q{qq.order}.</b>
            <span dangerouslySetInnerHTML={{ __html: qq.question_html }} />
          </div>
          <div style={{ margin: "6px 0 0 18px" }}>
            {qq.options.map((o) => {
              const correct = o.key === qq.correct_option;
              const yours = o.key === qq.your_option;
              return <div key={o.key} style={{ color: correct ? "#15803d" : yours ? "#b91c1c" : "#334155", fontWeight: correct || yours ? 600 : 400 }}><span>{o.key}.</span> <span dangerouslySetInnerHTML={{ __html: o.html }} />{correct ? " ✓" : yours ? " (your answer)" : ""}</div>;
            })}
          </div>
          ...
          {qq.explanation_html && <div style={{ margin: "4px 0 0 18px", fontSize: 12, background: "#f8fafc", padding: 8, borderRadius: 6 }} dangerouslySetInnerHTML={{ __html: `<b>Explanation:</b> ${qq.explanation_html}` }} />}
        </div>
      ))}

      <p style={{ marginTop: 20, fontSize: 11, color: "#94a3b8", textAlign: "center" }}>{data.disclaimer}</p>
    </div>
  );
}
