...
import type { InstallmentReminderPreview } from "@/lib/sms/installmentReminderService";

/**
 * Bulk installment reminders: sticky action bar → review screen → explicit send.
 *
...
export default function BulkInstallmentReminder({
  ...
  selectedIds: string[];
  /** Mirrors the page's "Overdue only" toggle so review matches the table. */
  overdueOnly: boolean;
  onClear: () => void;
  /** Called after a send so the table can refresh its tracking pills. */
  onSent: () => void;
}) {
  ...
  const openReview = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/admin/sms/installment-reminder/bulk", {
        ...
        body: JSON.stringify({ enrollmentIds: selectedIds, overdueOnly }),
      });
  ...
  function downloadCsv() {
    ...
    a.download = `installment-reminders-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      {open && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/40 p-2 sm:p-4" onClick={close}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Review installment reminders"
          >
            {/* ---- header: totals, exclusions, DLT id ---- */}
            <div className="flex items-start justify-between gap-3 border-b border-line p-4">
              <div>
                <h3 className="text-base font-bold">Review installment reminders</h3>
                {preview && (
                ...
                return (
                  <div key={p.enrollmentId} className={`mb-3 rounded-xl border border-line p-3 ${isExcluded ? "opacity-50" : ""}`}>
                    ...
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-ink2">
                      ...
                      <span className={p.daysOverdue > 0 ? "text-danger" : ""}>
                        {p.daysOverdue > 0 ? `${p.daysOverdue}d overdue` : "not overdue"}
                      </span>
