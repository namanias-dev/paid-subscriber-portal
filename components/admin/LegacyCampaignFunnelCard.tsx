 ...
 * Collapsed by default to a single hero total; expand on click for the chart.
...
export default function LegacyCampaignFunnelCard() {
  ...
  const visible = listExpanded || !hasMore ? sorted : sorted.slice(0, TOP_CAMPAIGNS_PREVIEW);
  const totals = data?.totals;
  const maxBar = Math.max(1, ...(visible.map((r) => r.matched)), totals?.matched || 0);
  const matched = loading ? null : totals?.matched ?? 0;
  ...
  return (
    <div className="card overflow-hidden">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-controls="legacy-campaign-funnel-panel"
        className="flex w-full items-start gap-3 p-4 text-left transition-colors hover:bg-surface2/40 motion-reduce:transition-none"
      >
        ...
        <ChevronDown
          size={18}
          aria-hidden="true"
          className={`mt-1 shrink-0 text-ink2 transition-transform motion-reduce:transition-none ${open ? "rotate-180" : ""}`}
        />
      </button>

      {open && (
        <div id="legacy-campaign-funnel-panel" className="border-t border-line px-4 pb-4 pt-3">
