/**
 * PRE-IMPORT SAFETY BACKUP — run BEFORE committing the Saarthi (Old) import.
 *
 ...
 *
 *   node --env-file=.env.local --import tsx scripts/backup-before-saarthi.ts
 *
 * Output: backups/saarthi-pre-import-<timestamp>/<table>.json  + _manifest.json
 */
...
async function main() {
  ...
  const dir = path.join(process.cwd(), "backups", `saarthi-pre-import-${stamp}`);
  fs.mkdirSync(dir, { recursive: true });

  const manifest: Record<string, unknown> = {
    label: "Saarthi (Old) pre-import safety backup",
