/**
 * READ-ONLY Excel report for the "Safalta June 2026 (Old)" legacy import.
 *
 ...
 *   node --env-file=.env.local --import tsx scripts/generate-safalta-report.ts
 */
...
const SOURCE_XLSX = path.join(os.homedir(), "Downloads", "SAFALTA JUNE BATCH - with Installment Pattern.xlsx");
const OUT_XLSX = path.join(os.homedir(), "Downloads", "Safalta-June-2026-Import-Report.xlsx");

// Identity-conflict phones held by the import (name mismatch vs existing account).
const HELD_IDENTITY: Record<string, { sheetName: string; existing: string }> = {
  "6264811772": { sheetName: "Pritam", existing: "Indu Dhurve" },
  "9781257203": { sheetName: "Amanpreet", existing: "Sharandeep Kaur" },
};
...
function parseSource(): RawRow[] {
  ...
  return json.map((r) => ({
    ...
    total: toNum(r["Total Course fee"]),
...
async function main() {
  ...
  const heldRows = src.filter((r) => (phoneCounts.get(r.phone) ?? 0) > 1 || HELD_IDENTITY[r.phone]);
  const s2Header = [
  ...
  const s2Rows = heldRows.map((r) => {
    ...
    if ((phoneCounts.get(r.phone) ?? 0) > 1) {
...
function buildStaffInstructions(courseTitle: string, held: RawRow[], stickers: Map<string, number>): (string | number)[][] {
  ...
  line("Safalta June 2026 — Staff Instructions (manual enrollment of held students)");
  line("");
  ...
  line("");
  line("STEP 0", "Get a correct, distinct mobile number for each held student FIRST (you cannot enroll without it).");
  line("", "• Duplicate-number pair (Shruti & Srushti): call both, ask each for their OWN mobile. You must end up with TWO different numbers.");
  line("", "• Name-mismatch (Pritam, Amanpreet): the number on the sheet already belongs to someone else in the portal.");
  line("", "  Call the student, confirm their real mobile. If the sheet number was wrong, use the corrected one.");
  line("", "• Never reuse a number that already belongs to a different person's account.");
  line("");
