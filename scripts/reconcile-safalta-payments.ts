/**
 * ONE-TIME reconcile for the "Safalta June 2026 (Old)" legacy import.
 *
 ...
 *   node --env-file=.env.local --import tsx scripts/reconcile-safalta-payments.ts
 *   node --env-file=.env.local --import tsx scripts/reconcile-safalta-payments.ts --commit
 *
 * Flags:
...
async function main() {
  ...
  console.log(hr("="));
  console.log(`  SAFALTA JUNE 2026 — PAYMENT RECONCILE   ${commit ? ">>> COMMIT (WRITES) <<<" : "DRY-RUN (writes nothing)"}`);
  console.log(hr("="));
