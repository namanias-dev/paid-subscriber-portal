/**
 * ONE-TIME legacy import — "Safalta June 2026 (Old)" batch.
 *
 * DRY-RUN by DEFAULT: writes NOTHING. Pass --commit to actually write.
 *
 * Usage:
 *   node --import tsx scripts/import-safalta-june.ts "<path-to-xlsx>"            # dry-run (default)
 *   node --import tsx scripts/import-safalta-june.ts "<path-to-xlsx>" --commit   # writes (needs approval)
 *
...
// ------------------------------- constants -------------------------------
const COURSE_SLUG = "safalta-june-2026-old";
const COURSE_TITLE = "Safalta June 2026 (Old)";
const DISCOUNT_REASON = "Legacy June 2026 negotiated fee";
const IMPORT_ACTOR = "legacy-import:safalta-june-2026";
...
// Sheet grand totals to reconcile against (all 83 rows, per locked decision).
const SHEET_EXPECTED = { collected: 1_862_300, pending: 1_759_700, effective: 3_622_000 };

/**
 * Phones explicitly HELD for staff action (NOT imported) — approved holds.
 * Deterministic so the hold applies even without a DB connection (dry-run). The
 * live-DB name-mismatch heuristic (enrichFromDb) is a redundant second guard.
 *   • Identity conflict: an existing portal account on this phone belongs to a
 *     differently-named person — staff must supply a correct, distinct number.
 */
const HELD_IDENTITY_PHONES: Record<string, string> = {
  "6264811772": 'Name mismatch — sheet "Pritam" vs existing account "Indu Dhurve". Staff must supply correct number.',
  "9781257203": 'Name mismatch — sheet "Amanpreet" vs existing account "Sharandeep Kaur". Staff must supply correct number.',
};
...
function parseRows(path: string): RawRow[] {
  ...
  return json.map((r, i) => ({
    ...
    mode: titleCase(String(r["Mode"] ?? "")),
    batch: titleCase(String(r["Batch"] ?? "")),
    status: String(r["Status"] ?? "").trim(),
    phone: digits(r["Mobile number"]),
    actualTotal: toNum(r["Total Course fee"]),
    paid: toNum(r["Total Fee received"]),
    pending: toNum(r["Pending Fee"]),
    nPending: toNum(r["Number of Instaments pending"]),
    eachMonth: toNum(r["Each month Installment amount"]),
  }));
}
...
async function ensureLegacyCourse(stickers: Map<string, number>, commit: boolean): Promise<Course | null> {
  ...
  return addCourse({
    ...
    title: COURSE_TITLE,
    category: "Foundation",
    description: "Legacy Safalta June 2026 batch — imported for portal access & fee tracking.",
...
function printHeader(commit: boolean, dbConnected: boolean, xlsxPath: string) {
  console.log(hr("="));
  console.log(`  SAFALTA JUNE 2026 (OLD) — LEGACY IMPORT   ${commit ? ">>> COMMIT (WRITES) <<<" : "DRY-RUN (writes nothing)"}`);
  console.log(hr("="));
...
function printTotals(all: StudentPlan[], imported: StudentPlan[]) {
  ...
  console.log(`  ALL ${all.length} SHEET ROWS (incl. skipped):`);
  line("Collected (Total Fee received)", sheetCollected, SHEET_EXPECTED.collected);
  line("Pending (Pending Fee)", sheetPending, SHEET_EXPECTED.pending);
  line("Effective total (Total Course fee)", sheetEffective, SHEET_EXPECTED.effective);
  console.log("");
  console.log(`  IMPORTED ${imported.length} STUDENTS (skipped removed):`);
  ...
  const allMatch =
    sheetCollected === SHEET_EXPECTED.collected &&
    sheetPending === SHEET_EXPECTED.pending &&
    sheetEffective === SHEET_EXPECTED.effective;
...
async function main() {
  ...
  if (!xlsxPath) {
    console.error('Usage: node --import tsx scripts/import-safalta-june.ts "<path-to-xlsx>" [--commit]');
    process.exit(1);
  }
  ...
  if (!totals.allMatch) {
    console.log(hr("!"));
    console.log("✗ DISCREPANCY: full-sheet totals do NOT match the locked sheet figures. STOPPING — do not commit until resolved.");
    console.log(hr("!"));
